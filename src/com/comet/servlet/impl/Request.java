...
import javax.servlet.http.HttpSession;
 ...
 public class Request {
	...
	private OutputStream  outputStream;
        ...
        private Map<String, ByteBuffer> contextSessionMap = new HashMap<String, ByteBuffer>();
   ...
   if(cookies != null){
       for(Cookie c:cookies){
         if(Constants.JSESSIONID.equals(c.getName())){
             ...
             if(session != null){
                 if(sessionCookieValue.equals(session.getId())){
                   ...
                   Map<String,SessionContext> map = cometState.getSerialializedSessionContextMap();
                   ...
                   if(map != null){
                       ...
                       System.out.println(" 2 === map -- " + map);
                        if(sc != null){
                            ...
                            if(session != null && sessionCookieValue.equals(session.getId())){
                              isSessionCreated = true;
                              session.setCometState(cometState);
                              return session; 
                            }
...
public HttpSession getSession1(){
       ...
       for(Cookie c:cookies){
         if(Constants.JSESSIONID.equals(c.getName())){
           //  ByteBuffer bb = cometState.getValue().get(context);
             ByteBuffer bb=null;
             System.out.println(" comet state get value " + cometState.getValue() + "  " + bb);
             if(bb == null){ // get it from containser context
            ...
            sc = CometUtil.getHttpSession(bb.array()); 
            if(sc != null){
               session = sc.getSession();
               isSessionCreated = true;
...
public HttpSession getSession(boolean isExist){
 ...
 private void processCookies(){
  ...
  public void setCometState(CometState state){
      cometState = state;
  }
