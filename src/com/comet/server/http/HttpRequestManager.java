 ...
 public final class HttpRequestManager {
   ...
   public void initializeCharBuffer(){
    ...
    public void processHeaderDataFromRequest(final ByteBuffer buffer,final Request request,final Response response,final SocketChannel channel){
      ...
      String headerStr =   reqStr.substring(reqStr.indexOf("\r\n")+ 2, reqStr.length());
      String reqResource = reqStr.substring(0,reqStr.indexOf("\r\n")); //GET /unni/servlets/images/return.gif HTTP/1.1
      String [] reqResources = reqResource.split(" ");
      String contextPath = reqResources[1];
      long start = System.currentTimeMillis();
      headerMap = HttpUtils.processRequestHeader(headerStr,"\r\n");
    ...
    // System.out.println(" what is there in the requestMap...." + requestMap);
     ...
     if(param.indexOf("&")  != -1){
        ...
        for(String s : params){
          ...
          String [] str = s.split("=");  
          requestMap.put(str[0].trim(), str[1].trim());
        }
     }else{
        if(param.contains("=")) {  
         params = param.split("=");
